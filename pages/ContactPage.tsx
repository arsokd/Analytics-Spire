          ...
          <div className="lg:col-span-2 p-10 relative bg-gray-900">
            ...
            {step === 'contact' && (
              <form onSubmit={handleSendOtp} className="space-y-8 animate-fadeIn">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="group"><label htmlFor="name" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Full Name</label><input id="name" type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full bg-transparent border-b border-gray-600 py-3 text-white text-lg focus:border-brand-500 focus:outline-none placeholder-gray-700" placeholder="Enter your name"/></div>
                  <div className="group"><label htmlFor="phone" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Phone</label><input id="phone" type="tel" name="phone" required value={formData.phone} onChange={handleChange} className="w-full bg-transparent border-b border-gray-600 py-3 text-white text-lg focus:border-brand-500 focus:outline-none placeholder-gray-700" placeholder="+91 ..."/></div>
                  <div className="md:col-span-2 group"><label htmlFor="email" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Email Address</label><input id="email" type="email" name="email" required value={formData.email} onChange={handleChange} className="w-full bg-transparent border-b border-gray-600 py-3 text-white text-lg focus:border-brand-500 focus:outline-none placeholder-gray-700" placeholder="name@company.com"/></div>
                </div>
            ...
            {step === 'otp' && (
              <form onSubmit={handleVerifyOtp} className="space-y-8 animate-fadeIn">
                ...
                <div><label htmlFor="otp" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">One-Time Password</label><div className="flex items-center border-b border-gray-600 focus-within:border-brand-500"><Lock className="text-gray-500 mr-3" size={20} /><input id="otp" type="text" name="otp" required value={otpInput} onChange={(e) => setOtpInput(e.target.value)} className="w-full bg-transparent py-3 text-white text-2xl tracking-[0.5em] font-mono focus:outline-none placeholder-gray-700" placeholder="XXXXXX" maxLength={6} /></div></div>
            ...
            {step === 'details' && (
              <form onSubmit={handleSubmitProfile} className="space-y-8 animate-fadeIn">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                   <div className="group"><label htmlFor="businessCategory" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Business Category</label><div className="relative"><select id="businessCategory" name="businessCategory" required value={formData.businessCategory} onChange={handleChange} className="w-full bg-gray-900 border-b border-gray-600 py-3 text-white text-lg focus:border-brand-500 focus:outline-none appearance-none"><option value="">Select...</option><option value="Manufacturing">Manufacturing</option><option value="Service">Service</option><option value="Other">Other</option></select><Building2 className="absolute right-0 top-3 text-gray-500 pointer-events-none" size={20} /></div></div>
                   <div className="group"><label htmlFor="employeeCount" className="block text-xs font-bold text-brand-500 uppercase tracking-widest mb-2">Employees</label><input id="employeeCount" type="number" name="employeeCount" required value={formData.employeeCount} onChange={handleChange} className="w-full bg-transparent border-b border-gray-600 py-3 text-white text-lg focus:border-brand-500 focus:outline-none"/></div>
